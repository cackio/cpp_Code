...
WebServer::WebServer(
            ...
            timer_(new HeapTimer()), threadpool_(new thread_pool(threadNum)), epoller_(new Epoller())
    {
...
void WebServer::DealRead_(HttpConn* client) {
    assert(client);
    ExtentTime_(client);
    threadpool_->add_task(std::bind(&WebServer::OnRead_, this, client)); // 这是一个右值，bind将参数和函数绑定
}
...
bool WebServer::InitSocket_() {
    ...
    // 优雅关闭
    {
    struct linger optLinger = { 0 };   //设置socket的linger选项，从而控制socket关闭时的行为。
    if(openLinger_) {
        /* 优雅关闭: 直到所剩数据发送完毕或超时 */
        optLinger.l_onoff = 1;    //表示启用linger选项，socket 关闭时会等待指定的时间（l_linger秒）以完成数据发送。
    ...
    ret = setsockopt(listenFd_, SOL_SOCKET, SO_LINGER, &optLinger, sizeof(optLinger));//设置 socket 选项
    if(ret < 0) {
        close(listenFd_);
